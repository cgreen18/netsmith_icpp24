...
class DatelineTransitionVCs(SimpleTopology):
    description='DatelineTransitionVCs'
    ...
    def makeTopology(self, options, network, IntLink, ExtLink, Router):
        ...
        flat_vc_mat_path = options.flat_vc_mat_file

        # its 2d
        # flat_vn_map = self.ingest_flat_map(flat_vn_map_path, n_routers)

        # its 3d
        flat_vc_mat = self.ingest_flat_3d(flat_vc_mat_path, n_routers)
        ...
        network.flat_src_dest_cur_to_vc = flat_vc_mat
        network.use_dateline_vcs = options.use_dateline_vcs
    ...
    def ingest_flat_3d(self, path_name, n_routers):
        print(f'ingesting 3d {path_name}')
        ...
        # print(f'in_list ({len(in_list)}) = {in_list}')
        # input('cont?')
